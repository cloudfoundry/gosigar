...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  install_go = <<-BASH
  install_go = <<-BASH
if [ ! -d "/usr/local/go" ]; then cd /tmp && wget https://storage.googleapis.com/golang/go1.3.3.linux-amd64.tar.gz && cd /usr/local && tar xvzf /tmp/go1.3.3.linux-amd64.tar.gz && echo 'export GOPATH=/home/vagrant/go; export PATH=/usr/local/go/bin:$PATH:$GOPATH/bin' >> /home/vagrant/.bashrc; fi
BASH
